...
- **Rooms & transitions.** One generic level host (`scenes/levels/level.tscn` — a
  `room_builder` Node3D + HUD + DialogueUI) builds whichever room
  `GameState.current_room` names, falling back to its `@export room_id` when that's
  empty (so the Phase 1 smoke still gets the greybox). A *threshold* door — a `door`
  entity with a `target_room` — emits `transition_requested` on open; `room_builder`
  records the target (`GameState.current_room` + `player_spawn`) and reloads the
  same scene via `SceneManager`, which rebuilds as the new room. The boot room is
  set in `scripts/core/main.gd`. `GameState.day` + `advance_day()` are the
  (persisted) day-cycle scaffold for the eventual return-to-booking loop.
...
- **New room:** add `data/rooms/<id>.json`; the generic level host
  (`scenes/levels/level.tscn`) builds it when `GameState.current_room` is its id
  (the boot room is set in `scripts/core/main.gd`; threshold doors switch rooms at
  runtime). Schema: `{id, ambient, objective?, size:[x,y,z], surfaces?:{floor,
  ceiling, walls}, spawns:{name:{pos,yaw}}, entities:[...]}` — `yaw` is in degrees,
...
After generating, tune the scenes' `@export` values in-editor. There is **one**
level scene, `scenes/levels/level.tscn` (the generic room host) — rooms are data,
so you never add a scene per room.
...
Run `gdformat scripts tests` to normalise indentation to tabs. Lint config lives
in `gdlintrc` (gdtoolkit defaults merged with a raised `max-public-methods` so the
auto-discovered `test_*` runner doesn't trip the cap).
